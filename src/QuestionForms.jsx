...
export function QuestionsForm({ sid, tag, qid, onDone, onCancel }) {
  const [f, setF] = useState({ question_text: '', options: ['', '', '', ''], correct_index: 0, explanation: '', difficulty: 1, source_year: '' });
  const [saving, setSaving] = useState(false);

  useEffect(() => { if (qid) supabase.from('questions').select('*').eq('id', qid).single().then(({ data: d }) => { if (d) setF({ question_text: d.question_text || '', options: d.options || ['', '', '', ''], correct_index: d.correct_index || 0, explanation: d.explanation || '', difficulty: d.difficulty || 1, source_year: d.source_year?.toString() || '' }); }); }, [qid]);

  async function submit(e) {
    e.preventDefault(); setSaving(true);
    const p = { question_text: f.question_text, options: f.options, correct_index: f.correct_index, explanation: f.explanation, difficulty: f.difficulty, source_year: f.source_year ? parseInt(f.source_year) : null, subject_id: sid, topic_tag: tag, format: 'single_choice', is_active: true, status: 'verified', updated_at: new Date().toISOString() };
...
export function BlitzForm({ sid, tag, qid, onDone, onCancel }) {
  const [f, setF] = useState({ text: '', is_true: true, explanation: '', difficulty: 1 });
  const [saving, setSaving] = useState(false);

  useEffect(() => { if (qid) supabase.from('blitz_questions').select('*').eq('id', qid).single().then(({ data: d }) => { if (d) setF({ text: d.text, is_true: d.is_true, explanation: d.explanation || '', difficulty: d.difficulty }); }); }, [qid]);

  async function submit(e) {
    e.preventDefault(); setSaving(true);
    const p = { text: f.text, is_true: f.is_true, explanation: f.explanation, difficulty: f.difficulty, subject_id: sid, topic_tag: tag, is_active: true, updated_at: new Date().toISOString() };
...
export function PairsForm({ sid, tag, qid, onDone, onCancel }) {
  const RID = ['А', 'Б', 'В', 'Г'];
  const [f, setF] = useState({ instruction: 'Встановіть відповідність', left: ['', '', '', ''], right: ['', '', '', ''], pairs: { '1': 'А', '2': 'Б', '3': 'В', '4': 'Г' }, explanation: '' });
  const [saving, setSaving] = useState(false);

  useEffect(() => { if (qid) supabase.from('logical_pairs_questions').select('*').eq('id', qid).single().then(({ data: d }) => { if (d) setF({ instruction: d.instruction, left: (d.left_items || []).map(i => i.text), right: (d.right_items || []).map(i => i.text), pairs: d.correct_pairs || { '1': 'А', '2': 'Б', '3': 'В', '4': 'Г' }, explanation: d.explanation || '' }); }); }, [qid]);

  async function submit(e) {
    e.preventDefault(); setSaving(true);
    const p = { instruction: f.instruction, left_items: f.left.map((t, i) => ({ id: String(i + 1), text: t })), right_items: f.right.map((t, i) => ({ id: RID[i], text: t })), correct_pairs: f.pairs, explanation: f.explanation, difficulty: 1, subject_id: sid, topic_tag: tag, is_active: true, updated_at: new Date().toISOString() };
...
export function GalleryForm({ sid, tag, qid, onDone, onCancel }) {
  ...
  return (
    <FormWrap title="Галерея" color="purple" onCancel={onCancel} onSubmit={submit} saving={saving}>
      <Field label="URL зображення"><input value={f.image_url} onChange={e => setF({ ...f, image_url: e.target.value })} className={inp} placeholder="https://..." /></Field>
...
export function SevensForm({ sid, tag, qid, onDone, onCancel }) {
  const [f, setF] = useState({ text: '', options: ['', '', '', '', '', '', ''], correct_answers: [0, 1, 2], explanation: '', difficulty: 1 });
  const [saving, setSaving] = useState(false);

  useEffect(() => { if (qid) supabase.from('seven_questions').select('*').eq('id', qid).single().then(({ data: d }) => { if (d) setF({ text: d.text, options: d.options || ['', '', '', '', '', '', ''], correct_answers: d.correct_answers || [0, 1, 2], explanation: d.explanation || '', difficulty: d.difficulty }); }); }, [qid]);
  ...
  async function submit(e) {
    ...
    const p = { text: f.text, options: f.options, correct_answers: f.correct_answers, explanation: f.explanation, difficulty: f.difficulty, subject_id: sid, topic_tag: tag, is_active: true, updated_at: new Date().toISOString() };
