...
const TEMPLATES = {
  questions: {
    ...
    headers: ['Тема', 'Текст питання', 'Варіант A', 'Варіант B', 'Варіант C', 'Варіант D', 'Правильна відповідь (A/B/C/D)', 'Пояснення', 'Складність (1/2/3)', 'Рік джерела'],
  ...
  blitz_questions: {
    name: 'Бліц (Так/Ні)',
    headers: ['Тема', 'Твердження', 'Правильна відповідь (ТАК/НІ)', 'Пояснення', 'Складність (1/2/3)'],
  ...
  logical_pairs_questions: {
    name: 'Логічні пари',
    headers: ['Тема', 'Інструкція', 'Ліва 1', 'Ліва 2', 'Ліва 3', 'Ліва 4', 'Права А', 'Права Б', 'Права В', 'Права Г', 'Пара 1→(А/Б/В/Г)', 'Пара 2→(А/Б/В/Г)', 'Пара 3→(А/Б/В/Г)', 'Пара 4→(А/Б/В/Г)', 'Пояснення'],
    example: ['Київська Русь', 'Встановіть відповідність', '1648', '1654', '1667', '1709', 'Повстання Хмельницького', 'Переяславська рада', 'Андрусівський мир', 'Полтавська битва', 'А', 'Б', 'В', 'Г', ''],
    parse: (row) => ({
      _topicName: (row['Тема'] || '').toString().trim(),
      ...
      right_items: [{ id: 'А', text: row['Права А'] || '' }, { id: 'Б', text: row['Права Б'] || '' }, { id: 'В', text: row['Права В'] || '' }, { id: 'Г', text: row['Права Г'] || '' }],
      correct_pairs: { '1': (row['Пара 1→(А/Б/В/Г)'] || 'А').toString().trim(), '2': (row['Пара 2→(А/Б/В/Г)'] || 'Б').toString().trim(), '3': (row['Пара 3→(А/Б/В/Г)'] || 'В').toString().trim(), '4': (row['Пара 4→(А/Б/В/Г)'] || 'Г').toString().trim() },
      explanation: row['Пояснення'] || '', difficulty: 1, is_active: true,
    }),
    validate: (item) => {
      if (!item._topicName) return 'Тема не вказана';
  ...
  seven_questions: {
    name: 'Сімки (3 з 7)',
    headers: ['Тема', 'Текст питання', 'Варіант 1', 'Варіант 2', 'Варіант 3', 'Варіант 4', 'Варіант 5', 'Варіант 6', 'Варіант 7', 'Правильний 1 (1-7)', 'Правильний 2 (1-7)', 'Правильний 3 (1-7)', 'Пояснення', 'Складність (1/2/3)'],
